#include "wordsolver_allappearances.h"
#include <iostream>
...
WordSolverAllAppearances::WordSolverAllAppearances()
{

}

WordSolverAllAppearances::WordSolverAllAppearances(vector<string> _dictionary, vector<vector<char>> _board)
{
    initBoard(_board);
    initDictionary(_dictionary);
}

void WordSolverAllAppearances::initDictionary(const vector<string> &_dictionary)
{
...
void WordSolverAllAppearances::initBoard(const vector<vector<char>>& _board)
{
    n = _board.size();
...
void WordSolverAllAppearances::solve()
{
    for(pair<const char, WordSolverAllAppearances::arbre*> it : arrel.fills){
...
//            = O(num-appearances)
void WordSolverAllAppearances::solve_rec(WordSolverAllAppearances::arbre* node)
{
    for (Appearance a : node->appearances){
        for (pair<const char, WordSolverAllAppearances::arbre*> it : node->fills){
...
void WordSolverAllAppearances::printSolution()
{
    cout << "Solutions:" << endl;
    for (pair<const char, WordSolverAllAppearances::arbre*> it : arrel.fills){
...
void WordSolverAllAppearances::print_rec(WordSolverAllAppearances::arbre* node, string word)
{
...
void WordSolverAllAppearances::insereix(string paraula, WordSolverAllAppearances::arbre* node)
{
    if (paraula.size() > 0) {
